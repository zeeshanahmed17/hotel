...
const RoomDetail = () => {
  ...
  const beds = room.beds || 1; // Default to 1 bed if not specified
  // If imageUrl exists use it, otherwise use image property
  const mainImage = room.imageUrl || room.image;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          ...
          <div className="flex flex-wrap gap-2 items-center text-neutral-600">
            <span className="inline-flex items-center">
              <BedDouble className="w-5 h-5 mr-2" /> {beds} {beds > 1 ? 'Beds' : 'Bed'}
            </span>
            <span className="inline-flex items-center ml-4">
              <Users className="w-5 h-5 mr-2" /> Up to {room.capacity || 2} Guests
            </span>
            <span className="inline-flex items-center ml-4">
              <Square className="w-5 h-5 mr-2" /> {room.size || 300} sq ft
            </span>
            <span className="inline-flex items-center ml-4">
        ...
        <div className="mt-4 md:mt-0">
          <Badge variant="outline" className="text-lg font-medium py-2 px-4 border-2 bg-gradient-to-r from-amber-600 to-yellow-400 text-white border-none">
            ${(room?.price || 0).toFixed(2)} <span className="text-sm">/night</span>
          </Badge>
        </div>
      </div>
              ...
              <div>
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <li className="flex items-start">
                    ...
                    <span>{beds} {beds > 1 ? 'beds' : 'bed'} with premium linens</span>
                  </li>
                  <li className="flex items-start">
                    <Check className="w-5 h-5 text-primary mr-2 mt-0.5" />
